...
class NoticeCreateSchema(BaseModel):
    ...
    notice_content: str | None = Field(
        default=None, max_length=65535, description="公告内容"
    )
    status: int = Field(
        default=0, ge=0, le=2, description="状态(0:草稿 1:已发布 2:已归档)"
    )
...
class NoticeQueryParam(BaseQueryParam, UserByQueryParam):
    ...
    notice_title: str | None = Field(
        None, description="公告标题", json_schema_extra={"q": "like"}
    )
    notice_type: str | None = Field(
        None, description="公告类型", json_schema_extra={"q": "eq"}
    )
