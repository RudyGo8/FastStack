...
async def get_online_list_controller(
    ...
    result_dict_list: list[dict[str, Any]] = await OnlineService.get_online_list(redis=redis, search=search)
...
async def get_current_online_controller(
    ...
    sessions: list[dict[str, Any]] = await OnlineService.get_current_user_sessions(redis=redis, user_id=auth.user.id)
...
async def get_dashboard_stats_controller(
    ...
    data: DashboardStatsSchema = await OnlineService.get_dashboard_stats(db=db, redis=redis)
