...
from app.modules.sop.config import AUTO_MERGE_ENABLED, AUTO_MERGE_THRESHOLD, LEAF_RETRIEVE_LEVEL
...
def auto_merge_chunks(results: list[dict], top_k: int = 5):
    ...
    if AUTO_MERGE_ENABLED_VALUE and results:
        try:
            # 统计次数
            parent_counts = Counter(item.get("parent_chunk_id", "") for item in results if item.get("parent_chunk_id"))
            merged_results = []
            # 记录已被合并的索引
            used_indices = set()
