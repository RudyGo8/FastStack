...
from app.modules.sop.rag.formatter import format_docs
from app.modules.sop.rag.pipeline import generate_hypothetical_document, retrieve_documents
...
def retrieve_initial(state: RAGState) -> RAGState:
    ...
    emit_rag_step(
        "🔎",
        "混合检索召回",
        (f"模式: {retrieve_meta.get('retrieval_mode', 'hybrid')}，候选 {retrieve_meta.get('candidate_k', 0)}"),
    )
    emit_rag_step(
        "🧩",
        "父子分块合并 (Auto-merge)",
        (f"启用: {bool(retrieve_meta.get('auto_merge_enabled'))}，应用: {bool(retrieve_meta.get('auto_merge_applied'))}，替换片段: {retrieve_meta.get('auto_merge_replaced_chunks', 0)}"),
    )
    emit_rag_step("✅", f"初次检索完成，保留 {len(results)} 个片段", f"重排: {bool(retrieve_meta.get('rerank_applied'))}")
...
def retrieve_expanded(state: RAGState) -> RAGState:
    ...
    if retrieved_hyde is not None:
        ...
        emit_rag_step(
            "🧱",
            "HyDE 扩展检索",
            (f"L{hyde_meta.get('leaf_retrieve_level', 3)} 召回，候选 {hyde_meta.get('candidate_k', 0)}，合并替换 {hyde_meta.get('auto_merge_replaced_chunks', 0)}"),
        )
    ...
    if retrieved_stepback is not None:
        ...
        emit_rag_step(
            "🧱",
            "Step-back 扩展检索",
            (f"L{step_meta.get('leaf_retrieve_level', 3)} 召回，候选 {step_meta.get('candidate_k', 0)}，合并替换 {step_meta.get('auto_merge_replaced_chunks', 0)}"),
        )
