...
from app.modules.sop.cache import cache
...
class ConversationStorage:
    ...
    def save(self, user_id: str, session_id: str, messages: list, metadata: dict | None = None, extra_message_data: list | None = None):
        ...
        try:
            # 查询 ChatSession 会话记录
            session = db.query(ChatSession).filter(ChatSession.username == user_id, ChatSession.session_id == session_id).first()
            if not session:
                session = ChatSession(username=user_id, session_id=session_id, metadata_json=metadata or {})
                db.add(session)
                db.flush()
            else:
                # 若已存在，更新元数据
            ...
            existing_rows = db.query(ChatMessage).filter(ChatMessage.session_ref_id == session.id).order_by(ChatMessage.id.asc()).all()
            ...
            can_append = len(existing_rows) <= len(incoming_rows) and all(_same_message(incoming_rows[idx], row) for idx, row in enumerate(existing_rows))
    ...
    def list_session_infos(self, user_id: str) -> list[dict]:
        ...
        db = SessionLocal()
        try:
            sessions = db.query(ChatSession).filter(ChatSession.username == user_id).order_by(ChatSession.update_time.desc()).all()
            result = []
    ...
    def get_session_messages(self, user_id: str, session_id: str) -> list[dict]:
        ...
        try:
            ...
            # 查询用户和会话
            rows = db.query(ChatMessage).filter(ChatMessage.session_ref_id == session.id).order_by(ChatMessage.id.asc()).all()
            result = [
                {
