from app.modules.sop.config import MODEL
from app.modules.sop.rag.runtime import get_last_rag_context
from app.modules.sop.tracing.mcp_trace import get_mcp_trace
...
def _build_mcp_summary(mcp_calls: list[dict] | None) -> dict:
    ...
    sources = sorted({str(item.get("server_name")).strip() for item in calls if isinstance(item, dict) and item.get("server_name")})
    return {
        "total": total,
        "success": success,
        "failed": failed,
