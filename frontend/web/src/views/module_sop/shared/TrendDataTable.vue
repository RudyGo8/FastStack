<template>
  <el-table :data="points" border size="small" max-height="420">
    ...
    <el-table-column label="销售出库数量" width="120" align="right">
    ...
    <el-table-column label="AI预测出库数量" width="130" align="right">
      <template #default="{ row }">{{ fmt(row.ai_forecast_outbound) }}</template>
    </el-table-column>
    <el-table-column label="AI预测激活数量" width="130" align="right">
      <template #default="{ row }">{{ fmt(row.ai_forecast_activation) }}</template>
    </el-table-column>
    <el-table-column label="提报预测出库数量" width="140" align="right">
      <template #default="{ row }">
        <span :class="{ 'diff-submit': row.submit_forecast && row.submit_forecast > 0 }">{{ row.submit_forecast ?? "—" }}</span>
      </template>
    </el-table-column>
...
<style scoped>
.diff-submit { color: #2563eb; font-weight: 600; }
</style>
